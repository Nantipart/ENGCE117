...
int main() {
    ...
    // Receive product details from user.
    ...
    // Check and display the profit status.
    if (total_profit > 0) {
...
// Function to calculate total profit based on stock quantity.
float calculate_total_profit(float cost, float sell, int qty) {
    return (sell - cost) * (float)qty;
}

// Function to calculate profit percentage per unit.
float calculate_profit_percentage(float cost, float sell) {
